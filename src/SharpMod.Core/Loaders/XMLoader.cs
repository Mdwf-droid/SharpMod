...
namespace SharpMod.Loaders
{
    ...
    public class XMLoader : ILoader
    {
        ...
        private readonly Stream _modStream;
            ...
            try
            {
                ...
                for (t = 0; t < mh.numpat; t++)
                {
                    XMPATHEADER ph = new XMPATHEADER
                    {
                        ...
                        size = Reader.ReadIntelULong(), //MmIO.Instance._mm_read_I_ULONG(this.ModStream);
                        packing = Reader.ReadUByte(),//MmIO.Instance.ReadUByte(this.ModStream);
                        numrows = (short)Reader.ReadIntelUWord(), //MmIO.Instance._mm_read_I_UWORD(this.ModStream);
                        packsize = Reader.ReadIntelUWord()// MmIO.Instance._mm_read_I_UWORD(this.ModStream);
                    };
                ...
                for (t = 0; t < this._module.Instruments.Count; t++)
                {
                    XMINSTHEADER ih = new XMINSTHEADER
                    {
                        ...
                        type = Reader.ReadUByte(), //MmIO.Instance.ReadUByte(this.ModStream);
                        numsmp = (short)Reader.ReadIntelUWord(),//MmIO.Instance._mm_read_I_UWORD(this.ModStream);
                        ssize = Reader.ReadIntelULong()//MmIO.Instance._mm_read_I_ULONG(this.ModStream);
                    };
